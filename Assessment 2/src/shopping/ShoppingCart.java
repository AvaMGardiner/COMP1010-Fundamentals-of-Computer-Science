...
public class ShoppingCart {
	...
	public boolean addItemBasic(CartItem itemToAdd) {
		if(itemCount == 100 || itemToAdd == null)
			return false;
		else
		{
	...
	public double getTotalSavings() {
		...
		for(int index = 0; index < itemCount; index++) {
			totalSaving = totalSaving + items[index].getTotalDiscount();
		}
	...
	public double getTotal() {
		...
		for(int index = 0; index < itemCount; index++) {
			totalCost = totalCost + items[index].totalCost();
		}
	...
	public boolean replace(int idx, CartItem itemToReplaceWith) {
		if(idx < 0 || idx >= itemCount || itemToReplaceWith == null)
			return false;
		else
	...
	public boolean removeItem(int idx) {
		...
		{
			for (int index = idx; index < itemCount; index++) {
	...
	public int reduceItemBy(String itemNameToReduce, int quantityToReduceBy) {
		for(int index = 0; index < itemCount; index++)
		{
			if(items[index].item.name.equalsIgnoreCase(itemNameToReduce)) {
				if(items[index].quantity <= quantityToReduceBy) {
					removeItem(index);
					return 0;
				}
				else
				{
					items[index].quantity = items[index].quantity - quantityToReduceBy;
					return 1;
				}
			}
		}
	...
	public ShoppingCart[] getGroupedByDiscount() {
		ShoppingCart[] groupedShoppingCart = new ShoppingCart[11];
		
		for(int index = 0; index < groupedShoppingCart.length; index++) {
			groupedShoppingCart[index] = new ShoppingCart();
		}
		
		for(int index = 0; index < itemCount; index++) {
			double currDiscount = items[index].discountPercentage;
			
			if(currDiscount >= 0 && currDiscount <= 9)
			{
				groupedShoppingCart[0].addItemBasic(items[index]);
			}
			else if(currDiscount >= 10 && currDiscount <= 19)
			{
				groupedShoppingCart[1].addItemBasic(items[index]);
			}
			else if(currDiscount >= 20 && currDiscount <= 29)
			{
				groupedShoppingCart[2].addItemBasic(items[index]);
			}
			else if(currDiscount >= 30 && currDiscount <= 39)
			{
				groupedShoppingCart[3].addItemBasic(items[index]);
			}
			else if(currDiscount >= 40 && currDiscount <= 49)
			{
				groupedShoppingCart[4].addItemBasic(items[index]);
			}
			else if(currDiscount >= 50 && currDiscount <= 59)
			{
				groupedShoppingCart[5].addItemBasic(items[index]);
			}
			else if(currDiscount >= 60 && currDiscount <= 69)
			{
				groupedShoppingCart[6].addItemBasic(items[index]);
			}
			else if(currDiscount >= 70 && currDiscount <= 79)
			{
				groupedShoppingCart[7].addItemBasic(items[index]);
			}
			else if(currDiscount >= 80 && currDiscount <= 89)
			{
				groupedShoppingCart[8].addItemBasic(items[index]);
			}
			else if(currDiscount >= 90 && currDiscount <= 99)
			{
				groupedShoppingCart[9].addItemBasic(items[index]);
			}
			else {
				groupedShoppingCart[10].addItemBasic(items[index]);
			}
		}
		
		return groupedShoppingCart;
	}
	...
	*    CartItem c4 = new CartItem(new StockItem("I1", 37.375), 1, 20);
	*    myCart.addItem(c4);
	*    object c1 should be retained at index 5 but quantity updated to 7
	*    (100% of 29.9 is equal to 80% of 37.375)
	*    
	...
	*/
	 ...
	 * 		I1 ($24 each, quantity 3, discounted by 10%) 
	 * 		I2 ($15 each, quantity 5, discounted by 15%)
	 * and two items in the parameter object cart: 
